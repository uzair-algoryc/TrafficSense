            ...
            for frame in frame_gen:
                ...
                for i in range(len(detections.xyxy)):
                    if hasattr(detections, 'tracker_id') and detections.tracker_id[i] is not None:
                        ...
                        # Use blue color for all vehicles (no wrong-way detection here)
...
def process_alpr_image(image_bytes: bytes) -> bytes:
...
async def alpr_image(file: UploadFile = File(None),image_path: str = Form(None)):
    """
    ...
    try:
        if file:
            ...
            # Convert relative /media/... path to absolute full path
        ...
        # Generate unique filename to prevent overwriting
        unique_processed_path = generate_unique_filename(str(PROCESSED_DIR), "processed", file.filename)
        processed_path = Path(unique_processed_path)
